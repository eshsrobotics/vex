...
class PivotRampPrototype:
  ...
  public:
    ...
    PivotRampPrototype(
        ...
        const std::vector<vex::motor>& intake,
        const std::vector<vex::motor>& lift,
        double rotationsToTop,
        const vex::digital_out& pneumaticsClamp
    );

    /**
     * Drive the robot at the given speeds.
     *
     * A fwd parameter of +1.0 means full speed forward.  -1.0 is full speed
     * reverse. A turn parameters of +1.0 turns full speed clockwise.  -1.0 is
     * full speed counterclockwise. drive = 0, turn = 0 stops the drive.
     */
