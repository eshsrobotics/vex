// The autonomous task tree with definitions for our robot's autonomous
// functions.
...
void execute(std::shared_ptr<Task> rootTask) {
    ...
    while (!taskList.empty()) {
        ...
        for (auto iter = taskList.begin(); iter != taskList.end();
            ...
            auto task = *iter;
            if (task->done()) {
                // Put the task's children into the list if they're ready (i.e.,
                // when all its parents are completed)
                for (auto childTask : task->children) {
                    bool childTaskReady = true;
                    for (auto parentTask : childTask->parents) {
                        // If the parent task doesn't exist, it is completed, so
                        // the child task won't wait for it
                        if (parentTask.expired()) {
                            continue;
                        }

                        // If one of our parents is running, then we are not
                        // ready.
                    ...
                    // If all the parents are completed or destroyed, the task
                    // will start
                    if (childTaskReady) {
                ...
                // This is safe if the task is a shared parent of a child task
                // that currently isn't ready
                iter = taskList.erase(iter);
            } else {
                // The current task is still running.  Move to the next task in
                // the list.
                ++iter;
            }
...
WaitMillisecondsTask::WaitMillisecondsTask(double timeToWaitMilliseconds) :
    Task("w") {
    // User passes in: 6000 (timeToWaitMilliseconds == 6000)
    // We currently have: 0 (waitTimeMilliseconds is default-initialized to 0)
    // Your code on the next line overwrites the 6000 with 0.  Is that what you
    // want?
...
DriveStraightTask::DriveStraightTask(
    double desiredDistanceCentimeters,
    Idrive& driveObject
) :
    Task("l"),
    predictedDistanceCm([=](double rotations) {
        return SLOPE * rotations + Y_INTERCEPT;
    }),
    distanceToDriveCm {desiredDistanceCentimeters},
    drive {driveObject} {}
...
bool TestDriveTask::done() const {
    ...
    // We want to edit these values but they are coming from a const function,
    // we const_cast to allow modification.
    ...
    // farther even if the motors are stopped. This is a much more precise way
    // of making sure the motors are stopped by checking if the change in motor
