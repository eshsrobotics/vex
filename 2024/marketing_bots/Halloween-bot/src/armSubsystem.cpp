...
void moveArm(
    ...
    if (armSpeedPercent != 0) {
        armMotorLeft
            .spin(directionType::fwd, armSpeedPercent, percentUnits::pct);
        armMotorRight
            .spin(directionType::fwd, -armSpeedPercent, percentUnits::pct);
    } else {
    ...
    // considered fully open, and 90 degrees is considered fully closed (for
    // now).
    ...
    // trap-jaw angle.
    const double CURRENT_CLAW_ANGLE_DEGREES =
        clawAngleWhenClosedDegrees - clawMotor.position(deg);
    ...
    switch (clawState) {
        ...
        case CLAW_OPEN:
            ...
            clawMotor.spinToPosition(
                CLAW_ANGLE_WHEN_OPEN_DEGREES,
                deg,
                CLAW_VELOCITY_PCT,
        ...
        case CLAW_CLOSE:
            ...
            // down on the triball.
            clawMotor.spinToPosition(
                clawAngleWhenClosedDegrees - CLAW_OVERBITE_ANGLE_DEGREES,
                deg,
                CLAW_VELOCITY_PCT,
                vex::velocityUnits::pct,
                false
