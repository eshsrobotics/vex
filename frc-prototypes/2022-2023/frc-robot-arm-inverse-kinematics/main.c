...
// If the power PID tells us to use is lower than this, don't bother moving.
// I do hate hearing the high-pitched whine that results when we tell a 269
// or 393 motor to move too slowly.
const int MOTOR_DEADZONE = 19;
...
const float armIncrementInches = 0.2;
...
task main()
{
    ...
    // least harm.
    setPID(shoulder, 1.500, 0.000, 1.500);
    setPID(elbow,    1.500, 0.000, 1.600);  // The elbow needs more oomph than the wrist
    setPID(wrist,    1.200, 0.000, 1.2500); // Not bad!
    ...
    while (1) {
        ...
        motor[ShoulderMotor] = abs(shoulderPower) < MOTOR_DEADZONE ? 0 : shoulderPower;
        motor[ElbowMotor] = abs(elbowPower) < MOTOR_DEADZONE ? 0 : elbowPower;
        motor[WristMotor] = abs(wristPower) < MOTOR_DEADZONE ? 0 : wristPower;
