...
#pragma config(Motor,  port9,           clawServo,     tmotorVex393_MC29, openLoop)
...
const int OPEN_CLAW_SERVO_ANGLE = 45;
const int CLOSE_CLAW_SERVO_ANGLE = 0;
...
void openClaw() {
    motor[clawServo] = OPEN_CLAW_SERVO_ANGLE;
}
...
void closeClaw() {
    motor[clawServo] = CLOSE_CLAW_SERVO_ANGLE;
}
...
task usercontrol()
{
  ...
  while (true)
  {
    ...
    // The claw is controlled by buttons 8R and 8D.  Since it is a servo,
    // there's no need for a condition to cut power to the clawMotor.
    if (vexRT[Btn8D] > 0) {
        openClaw();
