...
void execute(std::vector<ScheduledOperation>& ops) {
  ...
  const double DRIVE_FORWARD_DURATION_MILLISECONDS = 2300;
  ...
  const double TRAY_PUSH_MILLISECONDS = START_ROTATE_LINEUP_MILLISECONDS + ROTATE_LINEUP_DURATION;
  const double TRAY_PUSH_DURATION_MILLISECONDS = 2100;
  ...
  std::vector<ScheduledOperation> blue_operations = {
      ...
      {START_DRIVING_STRAIGHT, DRIVE_FORWARD_START_MILLISECONDS / 1000.0, 45},
      ...
      {START_TRAY, TRAY_PUSH_MILLISECONDS / 1000.0, 25},
      ...
      {START_INTAKE, (TRAY_PUSH_MILLISECONDS + TRAY_PUSH_DURATION_MILLISECONDS + 500) / 1000.0, -50},
