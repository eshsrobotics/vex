...
void execute(std::vector<ScheduledOperation>& ops) {
  ...
  const double INTAKE_LIFT_DURATION_MILLISECONDS = 650;
  ...
  const double TRAY_PUSH_DURATION_MILLISECONDS = 2500;
  ...
  std::vector<ScheduledOperation> red_operations = {
      ...
      {START_TRAY, (TRAY_PUSH_MILLISECONDS + TRAY_PUSH_DURATION_MILLISECONDS) / 1000.0, -20},
